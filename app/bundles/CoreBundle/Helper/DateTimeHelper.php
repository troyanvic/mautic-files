...
class DateTimeHelper
{
    ...
    public function toLocalString($format = null)
    {
        if ($this->datetime) {
            ...
            // translate each month to German
            switch ( $output = $local->format( $format ) ) {
                case strpos( $output, 'January' ) !== false:
                    return str_replace( 'January', 'Januar', $output );

                case strpos( $output, 'February' ) !== false:
                    return str_replace( 'February', 'Februar', $output );

                case strpos( $output, 'March' ) !== false:
                    return str_replace( 'March', 'März', $output );

                case strpos( $output, 'May' ) !== false:
                    return str_replace( 'May', 'Mai', $output );

                case strpos( $output, 'June' ) !== false:
                    return str_replace( 'June', 'Juni', $output );

                case strpos( $output, 'July' ) !== false:
                    return str_replace( 'July', 'Juli', $output );

                case strpos( $output, 'October' ) !== false:
                    return str_replace( 'October', 'Oktober', $output );

                case strpos( $output, 'December' ) !== false:
                    return str_replace( 'December', 'Dezember', $output );
