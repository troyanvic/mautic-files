...
Mautic.generatePageTitle = function(route){
    ...
    if (-1 !== route.indexOf('timeline')) {
        ...
        mQuery('title').html( currentModule[0].toUpperCase() + currentModule.slice(1) + ' | ' + currentModuleItem + ' | Marketing Monkeys' );
    } else {
        //loading basic title
        mQuery('title').html( mQuery('.page-header h3').html() + ' | Marketing Monkeys' );
    }
};
